...
export default function ItemEditable({
  ...
  function StockRow({ color }: { color: string }) {
    const stockErrors = validationErrors.filter((error) => {
      return (
        error.path?.startsWith("stockCount") && error.path.endsWith("count")
      );
    });
  ...
  function DraggableColorImages({ color }: { color: string }) {
    ...
    const mediaError = validationErrors.find(
      (error) => error.path === "mediaObjects"
    )?.message;
      ...
      <>
        <div className="item-image-edit-row">
          ...
          <label htmlFor={`item-image-add-${itemId}-${color}`}>
            <div
              className={`item-edit-image-add ${
                loadingNewImage ? "disabled" : ""
              }`}
            >
              <AddIcon />
          ...
          {item.stockCount?.length !== 0 ? (
            <TableContainer component={Paper}>
              <Table sx={{ minWidth: 650 }}>
                ...
                <TableBody>
                  {item.colorOptions?.map((color) => StockRow({ color }))}
                </TableBody>
              </Table>
            </TableContainer>
          ) : (
