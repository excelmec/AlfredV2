...
export function useCa() {
	...
	async function fetchCa(ambassadorId: number) {
		try {
			...
			const pointLog = await axiosEventsPrivate.get<CaPointLog[]>(
				`/api/pointlogs/${ambassadorId}`
			);
	...
	async function deletePoint(pointId: number) {
		try {
			...
			await axiosEventsPrivate.delete<CaPointLog>(
				`/api/pointlogs/delete/${pointId}`
			);
