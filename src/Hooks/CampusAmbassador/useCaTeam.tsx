...
export function useCaTeam() {
	...
	async function fetchCaTeam(teamId: number) {
		try {
			setLoading(true);
			setError('');
			const teamResponse = await axiosEventsPrivate.get<CaTeamEvents>(
				`/api/cateams/${teamId}`
			);
			...
			const CaListEventsResponse = await axiosEventsPrivate.get<
				CaListRes[]
			...
			setCaTeam({
				...teamResponse.data,
				ambassadors: teamResponse.data?.ambassadors?.map(
					(ambassador) => {
						return {
							...ambassador,
